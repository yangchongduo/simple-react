```js
  // element 是ReactDom.render(element,)//第一参数
  function updateContainer(element, container, parentComponent, callback) => {
    // 这里在初次渲染时 element是children container 是 root parentComponent是null
    let current = container.current // current 就是 RootFiber 第一次渲染的时候是 unintallFiber
